...
public class BookingServiceImpl implements BookingService{
    ...
    public Booking bookCab(Long sourceCityId, Long destinationCityId) {
        ...
        List<Cab> eligibleCabs = cabRepository.findAll()
                .stream()
                .filter(cab -> cab.getState() == CabState.IDLE)
                .filter(cab -> cab.getCity().getCityId().equals(sourceCityId))
                .toList();
        ...
        assignedCab.setLastStateChangeTime(Instant.now());
        cabRepository.save(assignedCab);

        Booking booking = new Booking(assignedCab.getCabId(), sourceCityId, destinationCityId); // source and destination same for simplicity
    ...
    public void completeTrip(Booking booking){
        Booking bookingExits = bookingRepository.findByBookingId(booking.getBookingId())
